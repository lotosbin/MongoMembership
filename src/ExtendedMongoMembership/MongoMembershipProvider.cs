...
namespace ExtendedMongoMembership
{
    public class MongoMembershipProvider : ExtendedMembershipProvider
    {
        ...
        internal static MembershipAccount GetUser(ISession session, string userName)
        {
        ...
        internal static int GetUserId(ISession session, string userTableName, string userNameColumn, string userName)
        {
            int result;
        ...
        private void CreateUserRow(ISession session, string userName, IDictionary<string, object> values)
        {
        ...
        private static bool SetPassword(ISession session, int userId, string newPassword)
        {
        ...
        private static int GetPasswordFailuresSinceLastSuccess(ISession session, int userId)
        {
        ...
        private bool CheckPassword(ISession session, int userId, string password)
        {
        ...
        private string GetHashedPassword(ISession session, int userId)
        {
            var user = session.Users.FirstOrDefault(x => x.UserId == userId);
        ...
        private int VerifyUserNameHasConfirmedAccount(ISession session, string userName, bool throwException)
        {
            var user = session.Users.FirstOrDefault(x => x.UserName == userName);
            if (user == null)
            {
